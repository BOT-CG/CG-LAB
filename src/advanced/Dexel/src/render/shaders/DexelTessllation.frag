...
void main()
{
    ...
    vec3 tempNormal = fs_in.normal;

    if(dot(tempNormal, directionLight) < 0.0){
        tempNormal = tempNormal * (-1.0);
    };
    

    // vec3 diffuse = colorLight * max(dot(fs_in.normal, directionLight), 0.0);
    vec3 diffuse = colorLight * max(dot(tempNormal, directionLight), 0.0);
    ...
    vec3 directionReflect = reflect(-directionLight, tempNormal); 
    ...
    vec3 specular = colorLight * pow(max(dot(tempNormal, directionHalfway), 0.0), 32.0);
   
    vec3 result = (ambient + diffuse + specular) * colorObject;//vec3(0.1,0.2,0.4);// fs_in.color ;//
