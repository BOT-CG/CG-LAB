...
bool NCProgram::execute(bool& shouldSubtract, DexelCutterSweptVolume& cutterSweptVolume)
{
...
bool NCProgram::execute(std::vector<DexelCutterSweptVolume>& cutterSweptVolumes)
{
    if (context.currentLine >= ncCodeLines.size()) {
        return false;
    }
    ...
    for (int i = 0; i < maxLinePerExecute && context.currentLine < ncCodeLines.size(); i++) {
        ...
        context.load(ncCodeLines[context.currentLine]);
        // std::cout << "context.currentLine: " << context.currentLine << std::endl;
        ...
        if (shouldSubtract) {
            ...
            // std::cout << "cutterSweptVolume: " << context.cutterSweptVolume.StartDexel << std::endl;
        }
    }
