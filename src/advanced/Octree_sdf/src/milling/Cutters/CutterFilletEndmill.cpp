...
float CutterFilletEndmill::signedDistance(const Posture& currentPosture, const Vector3Df& p) const
{
    ...
    return SignedDistanceFunctions::roundedCylinder(p, radius(), height, filletRadius, currentPosture.center + currentPosture.direction.normalize() * height / 2.0f, currentPosture.direction.normalize());
}
